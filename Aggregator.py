def pipeline(ev_day,ev_num,ev_count,evcs,evcs_plug,evcs_num,evcs_tot,tou,time_slot,time):
    ...
    ## 변수 선언 : 효율 때문에 충방전 나눔.
    Power_pos = [m.addVars(range(ev_day['dur'][vdx]), ub=evcs['Pmax'][vdx+((time_slot.day-1)*len(ev_day))], lb= 0, vtype = GRB.INTEGER) for vdx in range(len(ev_day))]
    Power_neg = [m.addVars(range(ev_day['dur'][vdx]), ub= 0, lb=evcs['Pmin'][vdx+((time_slot.day-1)*len(ev_day))], vtype = GRB.INTEGER) for vdx in range(len(ev_day))]
    ...
    m.optimize()
    # 결과 정리
    Power = pd.DataFrame(np.zeros([time,len(ev_day)]))
